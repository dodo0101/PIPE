...
public class PipeApplicationView extends JFrame implements ActionListener, Observer {
			...
			public void propertyChange(PropertyChangeEvent evt) {
				if (evt.getPropertyName().equals(PipeApplicationModel.TOGGLE_ANIMATION_MODE)) {
					...
					if (petriNetTab != null) {
						...
						SelectionManager selectionManager = applicationController.getActivePetriNetController()
								.getSelectionManager();
	...
	/**
	 * Refreshes the combo box that presents the Tokens available for use. If there
	 * are no Petri nets being displayed this clears it
	 */
	...
	public void registerNewPetriNet(PetriNet petriNet) {
		...
		petriNet.addPropertyChangeListener(new PropertyChangeListener() {
			...
			public void propertyChange(PropertyChangeEvent evt) {
				...
				} else if (msg.equals(PetriNet.NEW_TOKEN_CHANGE_MESSAGE)
						|| msg.equals(PetriNet.DELETE_TOKEN_CHANGE_MESSAGE)) {
					refreshTokenClassChoices();
				}
			}
		});
		...
		PetriNetController petriNetController = applicationController.getActivePetriNetController();
		petriNetTab.setMouseHandler(new PetriNetMouseHandler(applicationModel, petriNetController, petriNetTab));
